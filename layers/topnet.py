...
import numpy as np
import math
from torch.nn import Sequential, Linear, ReLU, Tanh, Conv1d, ModuleList
...
class topnet(nn.Module):
    def __init__(self, z_dim, nlevels, node_feat_dim, num_output_points):
        ...
        self.last_mlp = Sequential(Conv1d(self.node_feat_dim, 3, 1), Tanh())

    def upconv(self, level):
        input_code_dim = self.input_code_dim
        output_code_dim = self.node_feat_dim
        up_ratio = self.tarch[level]
        return Sequential(Conv1d(input_code_dim, int(input_code_dim / 2), 1), ReLU(),
                          Conv1d(int(input_code_dim / 2), int(input_code_dim / 4), 1), ReLU(),
                          Conv1d(int(input_code_dim / 4), int(input_code_dim / 8), 1), ReLU(),
                          Conv1d(int(input_code_dim / 8), output_code_dim * up_ratio, 1), Tanh()
                            )
