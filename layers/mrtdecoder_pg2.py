...
class MRTDecoderPGV2(MRTDecoder):
    def __init__(self, z_dim, nlevels, feat_dims, num_output_points):
        super(MRTDecoderPGV2, self).__init__(z_dim, nlevels, feat_dims, num_output_points)
    ...
    def forward(self, z, phase, alpha):
        ...
        if phase == 0:
            curr_phase_node = tree_node_list[-1][0]
            pc = self.toPoints[phase](curr_phase_node)
        elif phase == self.nlevels:
            node_concat = self.concat_multires(tree_node_list[-1])
            pc = self.final_conv(node_concat)
        else:
            prev_phase_node = tree_node_list[-2][0]
