...
class AETestManager(TestManager):
    ...
    def print_configs(self):
        super(AETestManager, self).print_configs()
        if self.is_pg:
            decoder_type = self.config['model']['decoder']['type']
            print(colored('num_points(pg) for ' + decoder_type + ' decoder: ' + str(self.num_points_of_phase), 'grey',
                          'on_green'))
    ...
    def get_id_and_data(self, data):
        # id & data
        input = data['pos'].cuda()
        if self.config['dataset']['type'] == 'surreal':
            id = data['id'][0]
            synset_id = 'surreal'
            model_id = id
        elif self.config['dataset']['type'] == 'shapenet':
            id = data['id'][0].split('.')[0]
            synset_id, model_id = id.split('_')
        else:
            raise NotImplementedError

        return input, synset_id, model_id

    def run(self):
        if self.config['dataset']['type'] == 'surreal':
            test_metrics = ['cd', 'f_score']
        else:
        ...
        for i, data in tqdm(enumerate(self.test_loader)):
            with torch.no_grad():
                # id & data
                input, synset_id, model_id = self.get_id_and_data(data)
                # inference
                if self.is_pg:
                ...
                for writer in test_metric_writers:
                    writer.run(temp_csv_row, output=recon, gt=input, synset_id=synset_id)
                csv_writer.writerow(temp_csv_row)

                # visualize
                if i % self.config['test_setting']['visu_period'] == 0:
                    plots_dir = self.plots_dir
                    if self.config['dataset']['class'] == 'multi':
                    ...
                    for i in range(len(num_colors) - 1):
                        if i == 0:
                    ...
                    plot_pcd(self.config, filename=os.path.join(plots_dir, '%s.png' % model_id),
                             pcds=pcds, titles=titles, suptitle=suptitle)
