...
class AETrainManager(TrainManager):
    ...
    def farthest_point_sampling(self, points):
        assert self.is_pg, 'farthest_point_sampling is only for progressive growing version.'

        batch_size, num_gt_points = points.shape[0], points.shape[1]
        num_points_of_curr_phase = self.num_points_of_phase[self.phase]

        points = points.view(-1, 3)  # (BN, 3)
        batch = torch.arange(batch_size).view(batch_size, 1).repeat(1, num_gt_points).view(-1).cuda()
        idx = fps(points, batch, ratio=num_points_of_curr_phase / num_gt_points)
        points = points[idx].view(batch_size, -1, 3)
        return points
    ...
    def run(self):
        ...
        for step in tqdm(range(self.global_step, max_step + 1)):
            ...
            if self.is_pg:
                starting_step_of_last_training_phase = self.cumulative_steps[-2]
                # fps for emd loss
                if self.config['train_setting']['loss'] == 'EMD' and step < starting_step_of_last_training_phase:
                    input = self.farthest_point_sampling(input)
    ...
    def eval(self, step, visu_period, visu_period_fine):
        ...
        for i, data in enumerate(self.valid_loader):
            ...
            if i % visu_period == 0:
                recon_num_colors = 8
                if self.is_pg:
                    starting_step_of_last_training_phase = self.config['train_setting']['pg_hp']['cumulative_steps'][-2]
                    # fps for emd loss
                    if self.config['train_setting']['loss'] == 'EMD' and step < starting_step_of_last_training_phase:
                        input = self.farthest_point_sampling(input)
                    recon = self.model(input, self.phase, self.alpha)
